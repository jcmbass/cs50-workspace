...
# Intitialize an empty expored set
explored = set()
...
def shortest_path(source, target):
    ...
    stack_frontier_path = search_frontier(stack_frontier, stack_frontier_path, target)
    queue_frontier_path = search_frontier(queue_frontier, queue_frontier_path, target)

    if len(stack_frontier_path) <= len(queue_frontier_path) and len(stack_frontier_path) != 0:
...
def search_frontier(frontier, path, target):

    #Keep looping until solution found for StackFrontier
    while True:
        # Clearing explored set
        explored.clear()
