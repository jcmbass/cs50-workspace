...
class MinesweeperAI():
            ...
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    if (i,j) not in self.moves_made and (i,j) not in self.safes:
                        new_knowledge.append((i,j))

                # Appending new Knowledge
                if len(new_knowledge) != 0:
                    self.knowledge.append(Sentence(new_knowledge,count))

                # Mark additional cells as safe or mines if possible
                while self.reduce_knowledge() != self.knowledge:
                    pass
    ...
    def make_random_move(self):
        ...
        available = board - self.moves_made - self.mines
        if len(available) > 0:
