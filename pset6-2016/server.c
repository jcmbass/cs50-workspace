...
char* indexes(const char* path)
{
    ...
    //allocate mamery to return a string
    char* index = malloc(strlen(path) + 10);
    
    // ensure index.php is readable
    if (access(strcat(strcat(index, path), php), R_OK) == -1)
    {
        memset(index, 0, strlen(index));
        
        // ensure index.html is readable
        if (access(strcat(strcat(index, path), html), R_OK) == -1)
        {
            memset(index, 0, strlen(index));
            free(index);
            return NULL;
...
const char* lookup(const char* path)
{
    // Prototype of formats and extensions
    const char point = '.';
    char* type = strrchr(path, point);
    char* css = ".css";
    char* html = ".html";
    char* javascript = ".js";
    char* php = ".php";
    ...
    else if(strcasecmp(type, javascript) == 0)
    {
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    //ensure only GET method is passed in request-line
    char* method = "GET ";
    int compared = strncmp(method, line, 4);
    ...
    //determine abs_path and query if there is
    int path_len;
    int qeu_len;
    int n, m;
    char* path = strchr(line, '/');
    char* qeu = strchr(line, '?');
    if(qeu == NULL)
    {
        query[0] = '\0';
    }
    
    //determine length of abs_path and query
    for(path_len = 0, n = strlen(path); path_len < n; path_len++)
    {
        if(path[path_len] == '"')
        {
            error(400);
        ...
        else if(path[path_len] == '?' || path[path_len] == ' ')
        {
            //passing abs_path
            strncpy(abs_path, path, path_len);
            abs_path[path_len] = '\0';
            break;
        }
    }
    ...
    if(qeu != NULL)
    {
        for(qeu_len = 0, m = strlen(qeu); qeu_len < m; qeu_len++)
        {
            if(qeu[qeu_len] == ' ')
            {
                //passing query
                strncpy(query, qeu + 1, qeu_len);
                query[qeu_len - 1] = '\0';
                break;
            }
        }
