...
int main(int argc, char* argv[])
{
    ...
    //resize BITMAPFILEHEADER and BITMAPINFOHEADER
    bi_out.biWidth *= n;
    bi_out.biHeight *= n;
    padding_out =  (4 - (bi_out.biWidth * sizeof(RGBTRIPLE)) % 4) % 4;
    bi_out.biSizeImage = ((sizeof(RGBTRIPLE)*bi_out.biWidth) + padding_out) * abs(bi_out.biHeight);
    bf_out.bfSize = sizeof(bf_out) + sizeof(bi_out) + bi_out.biSizeImage;
    ...
    //variables to iterate multiple times in on scanline
    int scanline = 0;
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        //return to first pixel in scanline
        if (scanline != n)
        {
